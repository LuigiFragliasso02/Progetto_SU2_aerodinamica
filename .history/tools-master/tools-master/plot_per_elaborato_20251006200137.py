...
def leggi_cp(filepath):
    ...
    dati_puliti = []
    for line in lines:
        # Sostituisci punti e virgola con virgola
        line = line.replace(';', ',')
        # Rimuovi caratteri strani tipo spazio non-breaking
        line = line.replace('\xa0', '').strip()
        # Dividi sui separatori multipli
...
def plot_cp(files, labels, outpath, title):
    ...
    for filepath, label in zip(files, labels):
        ...
        if not df.empty:
            plt.plot(df["x"], df["cp"], label=label)
    plt.gca().invert_yaxis()  # Cp va verso il basso
    plt.xlabel("x/c")
    plt.ylabel("$C_p$")
    plt.title(title)
    plt.legend()
...
if __name__ == "__main__":
    ...
    "FREE_TRANSITION_0012/dati_numerici_free_trans_i0_003.csv",
    "FREE_TRANSITION_0012/NOSTRI_DATI_SPERIMENTALI_ABBOTT/RE=6/DATI_SPERIMENTALI_ABOTT_RE6.csv"
    ]
